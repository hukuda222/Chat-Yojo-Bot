...
parser.add_argument('--out-type', '-o', choices=['hsm', 'ns', 'original'],
                    default='hsm',
                    help='output model type ("hsm": hierarchical softmax, "ns": negative sampling, "original": no approximation)')
...
class ConinuousBoW(chainer.Chain):
    def __init__(self, n_vocab, n_units, loss_func):
        super(ConinuousBoW, self).__init__(
...
class WindowIterator(chainer.dataset.Iterator):
    def __init__(self, dataset, window, batch_size, repeat = True):
        ...
        self.order = np.random.permutation(len(dataset) - window * 2)
        self.order += window
...
def main():
    ...
    if args.test:
        train = train[:100]
        val = val[:100]
    vocab = chainer.datasets.get_ptb_words_vocabulary() # dict which maps word2index
    index2word = {wid: word for word, wid in six.iteritems(vocab)} # dict which maps index2word
